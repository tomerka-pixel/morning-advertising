...
- בנה את הפוסט לפי מסגרת הכתיבה שנבחרה:
  · מוביל לפעולה = קודם עוצרים את הקורא, אחר כך מסקרנים, אחר כך גורמים לו לרצות, ורק בסוף מבקשים.
  · מהכאב לפתרון = מציגים את הבעיה שהלקוח מכיר, מחדדים אותה קצת, ואז אתם הפתרון.
  · סיפור קצר = רגע אנושי אחד וקטן שממחיש את הערך, בלי למכור ישירות.
- התאם את הטון והאורך לפלטפורמה: אינסטגרם וטיקטוק: קליל, קצבי ואישי. פייסבוק: זורם וחברותי. לינקדאין: מקצועי ומהוקצע.
...
- אסור להשתמש בשום מקף מפריד: לא מקף ארוך (—) ולא מקף בינוני (–). במקומם פסיק, נקודה או שורה חדשה.
...
- אסור מקף מפריד מכל סוג (— או –). אין אנגלית מלבד שם המותג. אין להמציא נתונים או המלצות.
...
- מטרה: ${b.goal || 'לא צוין'}
- טון: ${b.tone || 'לא צוין'}
- מסגרת כתיבה: ${b.framework || 'מוביל לפעולה'}
